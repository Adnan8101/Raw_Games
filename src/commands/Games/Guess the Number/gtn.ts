...
import { SlashCommandBuilder, ChatInputCommandInteraction, PermissionFlagsBits, TextChannel, EmbedBuilder, MessageFlags, ThreadChannel, GuildMember } from 'discord.js';
...
export async function execute(interaction: ChatInputCommandInteraction, services: any) {
    ...
    if (!manager) {
        manager = GuessTheNumberManager.getInstance(interaction.client);
    }
