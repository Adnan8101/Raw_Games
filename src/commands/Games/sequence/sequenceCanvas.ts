...
export class SequenceCanvas {
    public static async generateImage(sequence: string, guildName: string, guildIconUrl: string | null): Promise<AttachmentBuilder> {
        const width = 800;
        const height = 300;
        ...
        for (let i = 0; i < 4; i++) {
            ...
            ctx.fillText(Math.floor(Math.random() * 10000).toString(), 0, 0);
            ctx.restore();
        }
        ...
        // Recalculate width with final font size for centering
        totalWidth = 0;
        for (const char of chars) {
            totalWidth += ctx.measureText(char).width + letterSpacing;
        }
        ...
        try {
            ...
            ctx.fillStyle = '#ffffff';
            ctx.textAlign = 'right';
            ctx.fillText(guildName, width - 20, 20);
        }
